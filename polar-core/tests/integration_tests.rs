...
macro_rules! qruntime {
    ($query:tt, $err:pat $(, $cond:expr)?) => {
        assert!(matches!(_qruntime(&mut Polar::default(), $query), ErrorKind::Runtime($err) $(if $cond)?));
    };
...
macro_rules! qparse {
    ($query:expr, $err:pat) => {
        assert!(matches!(
            Polar::default().load_str($query).unwrap_err().kind,
            ErrorKind::Parse($err)
        ));
    };
}
...
#[test]
fn test_lookup() {
    qeval(&mut Polar::default(), "{x: 1}.x = 1");
}
...
fn test_instance_lookup() {
    ...
    qext(&mut Polar::default(), "new a(x: 1).x = 1", values![1], 1);
}
...
fn test_make_external() -> TestResult {
    let q = Polar::default().new_query("x = new Bar(1, a: 2, b: 3)", false)?;
