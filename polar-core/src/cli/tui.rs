...
impl App {
    ...
    fn update_state(&mut self) {
        if let Some(ref mut query) = self.query {
            for (k, v) in query.top_runnable().get_vm().unwrap().bindings(true) {
                self.bindings.push(format!("{}: {}", k, v));
            }
            for choice in &query.top_runnable().get_vm().unwrap().choices {
                self.choices.push(format!("{}", choice));
            }
            for goal in query.top_runnable().get_vm().unwrap().goals.deref() {
    ...
    fn result<E: std::error::Error>(
        ...
        match res {
            Some(Ok(QueryEvent::Done { result })) => {
            ...
            Some(Ok(QueryEvent::Result { bindings, .. })) => {
                ...
                for (k, v) in self.query.as_ref().unwrap().vm.bindings(false) {
                    self.messages
            ...
            Some(Ok(QueryEvent::Debug { message })) => self.messages.push(message),
        ...
        if self.query.is_some() {
            ...
            loop {
                match event::read()? {
                    ...
                    event::Event::Key(_) => {
                        self.query.as_mut().unwrap().debug_command("g").unwrap();
                        break;
                    }
                    _ => {}
                }
            ...
            let mut query = self.polar.new_query(&input, false).unwrap();
            query.debug_command("g").unwrap();
            self.clear();
...
fn display_messages<B: backend::Backend>(
    ...
    let message_list = List::new(messages.iter().rev().take(6).rev().map(Text::raw))
