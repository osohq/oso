        ...
        match t.value() {
            ...
            Value::Variable(v) => {
                ...
                if self.seen.contains(&hash) {
                    ...
                    let t = self.binding_manager.lookup(&v.name).unwrap_or(t);
...
impl BindingManager {
    ...
    pub fn new_with_kb(kb: Arc<RwLock<KnowledgeBase>>) -> Self {
        ...
        for (k, v) in kb.read().unwrap().get_registered_constants() {
            bm.bind(k, v.clone()).unwrap().is_none();
        }
        bm
    }
    ...
    fn partial_bind(&mut self, partial: Operation, var: &Symbol, val: Term) -> Result<Term> {
    ...
    pub fn bind(&mut self, var: &Symbol, val: Term) -> Result<Option<Term>> {
        use BindingManagerVariableState::*;
        let mut goal = None;
...
impl BindingManager {
    ...
    fn bind_variables(&mut self, left: &Symbol, right: &Symbol) -> Result<Option<Term>> {
        use BindingManagerVariableState::*;
        match (
