...
impl<'kb> Folder for Renamer<'kb> {
    fn fold_variable(&mut self, v: Variable) -> Variable {
        if self.kb.is_constant(&v.name) {
            v
        } else if let Some(w) = self.renames.get(&v.name) {
            Variable::new(w.0.clone())
        } else {
            let w = self.kb.gensym(&v.name.0);
            self.renames.insert(v.name, w.clone());
            Variable::new(w.0)
        }
    }
}
...
impl<'kb> Folder for Rewriter<'kb> {
    ...
    fn fold_term(&mut self, t: Term) -> Term {
        match t.value() {
            ...
            Value::Expression(o) if self.needs_rewrite(o) => {
                ...
                let temp = Value::Variable(Variable::new(self.kb.gensym(temp_name(&o.operator)).0));
    ...
    fn fold_variable(&mut self, v: Variable) -> Variable {
        if &v.name.0 == "_" {
            Variable::new(self.kb.gensym("_").0)
        } else {
            v
        }
    }
