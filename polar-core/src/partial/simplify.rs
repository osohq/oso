...
impl Folder for VariableSubber {
    fn fold_variable(&mut self, v: Variable) -> Variable {
        if v.name == self.this_var {
            Variable::new("_this".to_string())
        } else {
            v
        }
    }
    ...
    match term.value() {
        Expression(o) if o.operator == Unify => {
            ...
            match (left.value(), right.value()) {
                (Variable(v), Variable(w)) if v == w => TRUE.into(),
                _ => term,
            }
        }
        _ => term,
...
impl Simplifier {
    ...
    pub fn deref(&self, term: &Term) -> Term {
        match term.value() {
            Value::Variable(var) => self.bindings.get(&var.name).unwrap_or(term).clone(),
            _ => term.clone(),
        }
    }
    ...
    fn is_output(&self, t: &Term) -> bool {
        match t.value() {
            Value::Variable(v) => self.output_vars.contains(&v.name),
            _ => false,
        }
    }
            ...
            Operator::Unify | Operator::Eq => {
                ...
                if left == right {
                    ...
                    match (left.value(), right.value()) {
                        ...
                        (Value::Variable(l), _)
                            if !self.is_bound(&l.name) && !self.is_output(left) =>
                        {
                            simplify_debug!("*** 1");
                            MaybeDrop::Bind(l.name.clone(), right.clone())
                        }
                        // Replace non-output variable r with left.
                        (_, Value::Variable(r))
                            if !self.is_bound(&r.name) && !self.is_output(right) =>
                        {
                            simplify_debug!("*** 2");
                            MaybeDrop::Bind(r.name.clone(), left.clone())
                        }
                        // Replace unbound variable with ground value.
                        (Value::Variable(var), val)
                            if val.is_ground() && !self.is_bound(&var.name) =>
                        {
                            simplify_debug!("*** 3");
                            MaybeDrop::Check(var.name.clone(), right.clone())
                        }
                        // Replace unbound variable with ground value.
                        ...
                        {
                            simplify_debug!("*** 4");
                            MaybeDrop::Check(var.name.clone(), left.clone())
                        }
        ...
        F: Fn(&mut Self, &mut Operation, &TermSimplifier) + 'static + Clone,
    {
        ...
        *term = de;
        ...
        match term.mut_value() {
            ...
            Value::List(list) => {
                for elem in list.elements.iter_mut() {
