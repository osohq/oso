    ...
    fn isa_expr(&mut self, left: &Term, right: &Term) -> PolarResult<()> {
        match right.value() {
            ...
            Value::Pattern(Pattern::Instance(InstanceLiteral { fields, tag })) => {
                ...
                let simplified = simplify_partial(var, partial.clone().into_term());
                ...
                self.choose_conditional(
                    ...
                    add_constraints.into_iter().map(|op| Goal::AddConstraint { term: op.into_term() }).collect(),
