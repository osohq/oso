...
def test_null_with_partial(engine, oso):
    ...
    assert str(posts) == (
        ...
        "posts.title AS posts_title, posts.access_level AS posts_access_level, "
        "posts.created_by_id AS posts_created_by_id, posts.needs_moderation AS "
        f"posts_needs_moderation \nFROM posts{where_clause}"
    )
    assert posts.count() == 0
