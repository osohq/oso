...
def register_models(oso: Oso, base_or_registry):
    """Register all models in registry (SQLAlchemy 1.4) or declarative base class
    (1.3 and 1.4) `base_or_registry` with Oso as classes.
    """
...
def authorize_model(oso: Oso, actor, action, session: Session, model):
    ...
    :param model: The model to authorize, must be an SQLAlchemy model or alias.
    """
