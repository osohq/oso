...
class FFIErrorNotFound(OsoError):
    """Raised when an error is generated by the oso Rust core, but the error type
    is not found.
    """
...
class PolarTypeError(PolarRuntimeError):
    """Error related to the type of a Polar object, generated by the Rust
    core.
    """
...
class UnregisteredClassError(PolarRuntimeError):
    """Raised on attempts to reference unregistered Python classes from a Polar
    policy.
    """
...
class DuplicateClassAliasError(PolarRuntimeError):
    """Raised on attempts to register a class with the same name as a class that
...
UNEXPECTED_EXPRESSION_MESSAGE = """Recieved Expression from Polar VM. The Expression type is only supported when
