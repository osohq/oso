...
impl Polar {
    pub fn new() -> Self {
        let inner = Rc::new(polar::Polar::new(None));
        Self {
            host: Host::new(Rc::downgrade(&inner)),
            inner,
            load_queue: Vec::new(),
        }
    }

    pub fn load_str(&mut self, s: &str) {
        self.inner.load(s).unwrap();
    }

    pub fn query_str(&mut self, s: &str) {
        let query = self.inner.new_query(s, false).unwrap();
        todo!()
    }
}
