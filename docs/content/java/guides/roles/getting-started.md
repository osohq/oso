...
weight: 2
description: >
    Get started with Oso Roles for Java
---
...
public class App {
  ...
  private void initOso() throws Exception {
    // create the Oso instance
    oso = new Oso();

    // register classes
    oso.registerClass(User.class);
    oso.registerClass(Org.class);
    oso.registerClass(OrgRole.class);
...
loaded with the `loadFile` method.
...
- `_type` is the Java class the resource definition is associated with.
