...
weight: 2
description: >
    Get started with Oso Roles for Go
---
...
application. Here's an example `App` class that creates a new `Oso`
instance on initialization:

```go
import (
  "github.com/osohq/go-oso"
  "github.com/osohq/go-oso/interfaces" // see under "Registering types" below
)
...
func (a *App) initOso() {
  ...
  o.RegisterClass(reflect.TypeOf(Record{}), nil)

  // load your policy file
  o.LoadFile("authorization.polar")

  // enable the build-in roles configuration
  o.EnableRoles()
...
The `Oso` type is the entrypoint to using Oso in our application.
We usually will have a global instance that is created
during application initialization and shared across requests.

### Registering types

Classes referred to in your policies must be registered with Oso
via the `RegisterClass` function. 

{{% callout "Type interfaces in Go" "blue" %}}

You'll need to implement the certain interfaces so Oso can compare different
instances of Go types. See the [Go quickstart app](/getting-started/quickstart)
for examples.
...
loaded with the `LoadFile` function.

### Enabling Oso roles

In order to enable the built-in roles features, we call the `EnableRoles` method
on the `Oso` instance.
...
- `_type` has the Go type the resource definition is associated with.
  **NOTE**: you must have registered this class with `RegisterClass` before
