...
Below is an example `resource` predicate that reflects the above changes. For a more in-depth example, see our [documentation]({{< ref path="/guides/roles/sqlalchemy" lang="python" >}}) and [example repository](https://github.com/osohq/gitclub/tree/main/backends/flask-sqlalchemy).
