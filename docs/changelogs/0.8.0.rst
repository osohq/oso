...
- More invalid expressions are captured at parse time. For example, you perform
...
- Matching against unbound variables now results in an error. Previously
  this would bind the variable with the pattern, resulting in unexpected errors.
  For example::
