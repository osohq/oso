...
def test_data(engine):
    ...
    public_user_post = Post(
        contents="public_user_post", access_level="public", created_by=user
    )
    private_user_post = Post(
        contents="private_user_post", access_level="private", created_by=user
    )
    private_manager_post = Post(
        contents="private manager post", access_level="private", created_by=manager
    )
    public_manager_post = Post(
        contents="public manager post", access_level="public", created_by=manager
    )

    models = {
        name: value for name, value in locals().items() if isinstance(value, Model)
    }
